(function() {
    ...
    function stringify(logger, method, args) {
        return args.map(function(arg) {
            return typeof arg === 'string' ? arg : inspect(arg);
        });
    }

    function prefix(logger, method, args) {
        var timestamp = new Date().toISOString();

        return ['{' + timestamp + '} [' + method + '] (' + logger.prefix() + ')'].concat(args);
    }

    function toConsole(logger, method, args) {
        /* global console */
