(function() {
    ...
    var logger = require('rc-logger').default.context('MRAID');
    ...
    var proxyMethod = (function() {
        var STATE_BLACKLIST = [STATE.LOADING, STATE.HIDDEN];

        return function proxyMethod(method) {
            return function proxy() {
                var $private = this.__private__;
                var state = $private.mraid.getState();

                if (STATE_BLACKLIST.indexOf(state) > -1) { return; }
    ...
    function MRAID(props) {
        ...
        this.waitUntilReady().then(function setProps() {
            ...
            var isMRAID2 = /^2/.test(version);
            var supportsInlineVideo = isMRAID2 ? $private.mraid.supports('inlineVideo') : undefined;
            ...
            logger.info('Ready! Version is', version, 'Supports inline video', supportsInlineVideo);
    ...
    Object.defineProperties(MRAID.prototype, {
        ...
        useCustomClose: {
            get: function getUseCustomClose() {
                var $private = this.__private__;
                var expandProps = $private.mraid.getExpandProperties();

                logger.log('Checking getExpandProperties()', expandProps);

                return expandProps.useCustomClose;
            }
        }
    });
