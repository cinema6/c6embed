    ...
    Selector.prototype = {
        ...
        classes: function() {
            return this.toArray().map(function(node) {
                var isAnimatedString = isObject(node.className) && 'baseVal' in node.className,
                    className = isAnimatedString ? node.className.baseVal : node.className;

                return className ? className.split(' ') : [];
            }).reduce(function(array, classes) {
                return array.concat(classes);
            }).filter(function(className, index, array) {
        ...
        css: function(config, value) {
            ...
            function set(config) {
                self.forEach(function(node) {
                    ...
                    for (prop in config) {
                        node.style[prop] = config[prop].toString();
                    }
                });
            }
