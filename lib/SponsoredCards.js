module.exports = function(deps) {
    ...
    _private.getCardConfigs = function(experience) {
        return (experience.data.deck || []).filter(function(card) {
            return card.sponsored === true && (
                ( (card.campaign && !!card.campaign.campaignId) || !!card.adtechId ) &&
                !( card.campaign && card.campaign.clickUrl && card.campaign.countUrl )
            );
        });
    };
    ...
    _private.makeAdCall = function(card, experience, pixels, placement, adtech, timeout) {
        var campaignId = String(card.campaign && card.campaign.campaignId || card.adtechId),
            bannerId = card.campaign && card.campaign.bannerId || card.bannerId || '1',
            deferred = q.defer();
            
        console.log('ASDF: Ad call for ' + campaignId + ', placement ' + placement); //TODO

        adtech.loadAd({
            placement: placement,
            params: { target: '_blank', adid: campaignId, bnid: bannerId },
            complete: function() {
                try {
    ...
    this.fetchSponsoredCards = function(experience, pixels, preloaded) {
        console.log('ASDF: calling fetchSponsoredCards');
        ...
        c6.addSponsoredCard = function(placementId, campaignId, extId, clickUrl, countUrl) {
